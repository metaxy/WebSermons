...
/**
 * Plugin 'All Sermons' for the 'websermons' extension.
 *
 * @author	 <>
 * @package	TYPO3
 * @subpackage	tx_websermons
 */
class tx_websermons_pfiles extends tslib_pibase {
	public $prefixId      = 'tx_websermons_pfiles';		// Same as class name
	public $scriptRelPath = 'pfiles/class.tx_websermons_pfiles.php';	// Path to this script relative to the extension dir.
	...
	protected function listView($content, array $conf) {
		...
		if (is_numeric($this->piVars['showUid'])) {	// If a single element should be displayed:
			...
			$this->internal['currentRow'] = $this->pi_getRecord('tx_websermons_files', $this->piVars['showUid']);
			...
			$this->internal['searchFieldList'] = 'title,speaker,topic,path,links';
			$this->internal['orderByList'] = 'uid,title,speaker,topic,path';
	
				// Get number of records:
			$res = $this->pi_exec_query('tx_websermons_files', 1);
...
if (defined('TYPO3_MODE') && isset($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/websermons/pfiles/class.tx_websermons_pfiles.php'])) {
	include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/websermons/pfiles/class.tx_websermons_pfiles.php']);
}
